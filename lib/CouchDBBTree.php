 ...
 * BTree
 */
final class CouchDBBTree
{
    /**
     * Under this key is last modification time of node
     */
    const MODKEY = "\xffm";
    ...
    {
        $size = self::SIZEOF_INT * 2 + self::SIZEOF_HEADER;
        if (fseek($this->handle, -$size, SEEK_END) === -1) return NULL;
        if (strlen($data = fread($this->handle, $size)) !== $size) return NULL;

        if (substr($data, self::SIZEOF_INT * 2) !== self::HEADER) {
            $all = $this->range("\x00", "\xff");
     ...
     * @param string
     * @param array NULL deletes given key
     * @return bool
     */
    public function set($key, $value)
    {
        // recount total rows
        if (($total = $this->total()) === NULL) return FALSE;
        $previous = $this->get($key);
        if      ($previous === NULL && $value === NULL) return TRUE;
        else if ($previous === NULL && $value !== NULL) $total++;
        else if ($previous !== NULL && $value === NULL) $total--;
        else if ($previous !== NULL && isset($value['_deleted']) && $value['_deleted'] === TRUE) $total--;

        // seek to end
        if (fseek($this->handle, 0, SEEK_END) === -1) return FALSE;
        if (($pos = ftell($this->handle)) === FALSE) return FALSE;
        $cursor = $pos;
        ...
        if (!(fflush($this->handle) &&
            self::header($this->handle, $root, $total) &&
            fflush($this->handle))) 
        {
            ftruncate($this->handle, $pos);
            flock($this->handle, LOCK_UN);
     ...
     * @param int get only leaves with last modifaction time greater than this
     *            (unix timestamp)
     * @return array pointers to leaves; NULL on failure
     */
    public function leaves($since = NULL)
    {
        ...
        do {
            ...
            foreach ($nodes as $_) {
                ...
                if ($since !== NULL && $node[self::MODKEY] <= $since) continue;
                unset($node[self::MODKEY]);
    ...
    public function compact()
    {
        ...
        if (!((
            ...
            self::header($compact, $root, $this->total()) !== FALSE && 
            fflush($compact) && 
    ...
    public static function open($filename)
    {
        ...
        if (ftell($handle) === 0) {
            ...
            $root = self::serialize(self::KVNODE, array(self::MODKEY => time()));
            $to_write = pack('N', strlen($root)) . $root;
            if (fwrite($handle, $to_write, strlen($to_write)) !== strlen($to_write) ||
                !self::header($handle, 0, 0) || !flock($handle, LOCK_UN)) 
            {
                ftruncate($handle, 0);
                fclose($handle);
                return FALSE;
     ...
     * @param string values count
     * @return bool
     */
    private static function header($handle, $root, $total)
    {
        $to_write = pack('N', $total) . pack('N', $root) . self::HEADER;
